...
def display(args, data_dir):
    ...
    print(("{} is blocking approximately {} domains ({}, e.g. {})".format(
        (os.path.join(args.host_folder, "hosts")),
        lc1 - lc2,
        "www-prefixed and non-www-prefixed",
        "www.ad-site.com and ad-site.com")))
...
def status_cmd(args):
    ...
    if(args.command == "original" or args.command == "off"):
        ...
        f1 = os.path.join(data_dir, "hosts.profile.{}".format(args.command))
        f2 = os.path.join(args.host_folder, "hosts")
...
def add_cmd(args):
    ...
    for fname in files:
        print(('Adding domains to {}'.format(fname)))
        f = open(fname, 'a')
...
def remove_cmd(args):
    ...
    files = ComposeFilesToModify(data_dir, args.type)
    for fname in files:
        print(('Removing domains from {}'.format(fname)))
        # remove file
        ...
        for i in range(0, len(lines)):
            for d in args.domain:
                for p in prefixes:
                    dstr = "127.0.0.1  " + p + d
                    if (not args.all_matching and
                        not args.load_previous_removes):
                        dstr += " "
...
def update_cmd(args):
    ...
    if(not os.path.exists(combine_hosts_path)):
        ...
        if(not os.path.exists(combine_hosts_path)):
            ...
            if(not os.path.exists(combine_hosts_path)):
                print(('Unable to find "{}"...'.format(args.exe_name)))
                sys.exit(1)
...
def main(argv):
    ...
    # Install option
    install_choice = subparser.add_parser("install",
        help="Install TrackingBlocker")
    install_choice.add_argument("files", nargs='*',
        help="Host files to install")
    install_choice.set_defaults(func=install_cmd)

    # Status option
    stat_choice = subparser.add_parser("status",
        help="TrackingBlocker status change/display")
    ...
    # Add option
    add_choice = subparser.add_parser("add",
        help="Add domain to TrackingBlocker")
    ...
    remove_choice.add_argument("-t", "--type",
        help="Tracking type to remove from",
        choices=['all', 'profile.0'],
        default="all")
    remove_choice.add_argument("-a", "--all-matching", action="store_true",
        help="Remove all domains starting with domains listed")
    remove_choice.add_argument("-l", "--load-previous-removes",
        action="store_true",
    ...
    update_choice.add_argument("-f", "--files",
        help="Files to include in update",
        nargs='*')
    update_choice.add_argument("-u", "--update",
        help="Update with WINHOSTS",
        action="store_true")
    update_choice.add_argument("-o", "--output-dir",
        help="temporary folder to hold outputs",
        default="/tmp/4wejsdfasdff")
    update_choice.add_argument("-d", "--default",
        help="default HOST file",
        required=True)
    update_choice.add_argument("-t", "--type",
        help="Tracking type to update",
        choices=['all', 'profile.0'],
        default="profile.0")
    update_choice.add_argument("--exe-path",
        help="Path to combine_hosts exe",
        default="../libexec")
    update_choice.add_argument("--exe-name",
        help="Alternative name to \"combine_hosts\"",
        default="combine_hosts")
    update_choice.add_argument("--rm-ctrl-M",
        help="Script to remove ^M characters",
        default="pyscripts")
...
if __name__ == "__main__":
    this_path = os.path.realpath(sys.argv[0])
    script_path = os.path.abspath(os.path.dirname(this_path))
